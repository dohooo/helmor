...
export class CursorCore {
	...
	/// streaming (#868). Non-auth (network/connection) faults may be
	/// connection-level, so they fall back to failing every in-flight turn (see
	/// `fatalScope`), never leaving a real victim hung.
	failTurnsForFatal(
		message: string,
		isAuth: boolean,
