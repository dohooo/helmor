...
/// List workspace changes alongside prefetched contents for every changed
/// (non-deleted, non-oversized) file. The remote runtime folds the change
/// list + the first batch of diffs into one round trip over SSH.
