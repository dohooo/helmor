...
export function ImagePreviewBadge({ path, onRemove }: { path: string; onRemove?: () => void }) {
  ...
  return (
    <>
      <span className="inline-flex items-center gap-1 rounded border border-app-border/60 text-[12px] transition-colors hover:border-app-foreground-soft/40 hover:bg-app-foreground/[0.03]">
        ...
        >
          <ImageIcon className="size-3 shrink-0 text-app-project" strokeWidth={1.8} />
          <span className="max-w-[200px] truncate text-app-foreground-soft">{fileName}</span>
        </button>
        {onRemove ? (
          <button
            type="button"
