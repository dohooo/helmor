...
export function SessionContextInjector({
	...
	return (
		<div className="pointer-events-auto mb-2 flex w-full items-center gap-2 self-start">
			<span className="shrink-0 text-[14px] font-normal leading-none text-foreground/70">
				Inject sessions:
			</span>
				...
				{candidates.map((session) => {
					...
					return (
						<button
							...
							className={cn(
								"grid h-5 max-w-[15rem] shrink-0 cursor-interactive grid-cols-[0.75rem_minmax(0,1fr)] items-center gap-1 rounded-md border px-1 text-[14px] leading-none transition-[background-color,border-color,color,box-shadow] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/60",
								isSelected
